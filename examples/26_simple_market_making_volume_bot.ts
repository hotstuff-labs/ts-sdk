...
const SIGNAL_WINDOW_MS = 20_000;

const BASE_ORDER_SIZE = 0.002;
const MAX_ORDER_SIZE = 0.01;
const MIN_SIGNAL_VOLUME = 0.03;

const BASE_HALF_SPREAD_BPS = 4;
const MAX_SKEW_BPS = 2;
...
function buildVolumeSignal(trades: TradePoint[]): VolumeSignal {
    let buyVolume = 0;
    let sellVolume = 0;
...
function roundToStep(value: number, step: number, mode: "up" | "down"): number {
    const steps = value / step;
    const roundedSteps = mode === "up" ? Math.ceil(steps) : Math.floor(steps);
    const decimals = decimalsFromStep(step);
    return Number((roundedSteps * step).toFixed(decimals));
}

function computeOrderSize(signal: VolumeSignal): number {
    const scale = 1 + Math.min(signal.total / MIN_SIGNAL_VOLUME, 4);
...
function buildQuoteOrders(
    ...
    const skewBps = signal.imbalance * MAX_SKEW_BPS;

    const bidDistanceBps = Math.max(1, BASE_HALF_SPREAD_BPS - skewBps);
    const askDistanceBps = Math.max(1, BASE_HALF_SPREAD_BPS + skewBps);

    let bidPrice = roundToStep(
        mid * (1 - bidDistanceBps / 10_000),
        tickSize,
        "down",
    );
    let askPrice = roundToStep(
        mid * (1 + askDistanceBps / 10_000),
        tickSize,
        "up",
    );

    if (askPrice <= bidPrice) {
        askPrice = Number((bidPrice + tickSize).toFixed(decimalsFromStep(tickSize)));
    }

    const size = computeOrderSize(signal);
    const priceDecimals = decimalsFromStep(tickSize);
    const sizeText = size.toFixed(SIZE_DECIMALS);
    const cloidBase = `mm-${Date.now()}`;
...
async function main() {
    ...
    try {
        ...
        while (Date.now() - startedAt < RUNTIME_SECONDS * 1_000) {
            ...
            pruneTrades(recentTrades);
            const signal = buildVolumeSignal(recentTrades);
            ...
            try {
                ...
                console.log(
                    `[quote] bid=${quote.bidPrice} ask=${quote.askPrice} size=${quote.size.toFixed(
                        SIZE_DECIMALS,
                    )} signal=${signal.imbalance.toFixed(2)} volume=${signal.total.toFixed(4)}`,
                );
            } catch (error) {
